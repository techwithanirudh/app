...
const HomePage: React.FunctionComponent = () => {
  ...
  const ImageCard = (props: { title: string; src: string; children?: any }) => (
    <div className="image-card w-full flex flex-col items-center">
  ...
  return (
    ...
    >
      ...
      <section className="z-10 text-center my-4">
        ...
        <div className="flex justify-center">
          <div className="lead font-light mt-2 max-w-xl">
            All-in-one easy-to-use text and voice chat. Stop paying for Discord
            boosts and hassling with Zoom.
            {Boolean(userCount) && (
              <span className="pl-1">
                Join{" "}
                <NumberFormat
                ...
                />{" "}
                Anichat users that simplified their life.
              </span>
            )}
          </div>
        </div>
        <button className="font-primary shadow-md bg-green text-dark">
          <Link to="/login">
            <NormalButton className="bg-white text-black rounded w-56 h-11 px-6 mt-8">
              Login
            </NormalButton>
          </Link>
        </button>
        <button className="shadow-md bg-green text-dark ml-10">
          <Link to="/channels/@me">
            <NormalButton className="text-black bg-success rounded W-56 h-11 px-6 mt-8">
              Open Anichat in Your Browser
            </NormalButton>
          </Link>
        </button>
      </section>
      ...
      <footer className="fixed bottom-0 w-full">
        <a
          href={`${process.env.REACT_APP_REPO_URL}/blob/dev/CHANGELOG.md`}
